# Durability, Concurrent Strength, and Heat Adaptation

Reference for Claude-as-coach: overlay concepts that complement the core block templates. Load this file when designing plans that involve strength integration, hot-weather events / tropical-climate riders, or long-duration target events.

- For core block templates and progressions → `references/block_templates.md`.
- For weekly adaptation decisions → `references/weekly_adaptation.md`.
- For taper structure → `references/race_taper.md`.

> **Citation currency** — heat-adaptation and concurrent-strength citations last verified **2026-05-20**. Re-verify against current literature at every skill audit. Anchor authorities: `governance/bibliography.md#ronnestad-bent` (strength → cycling neuromuscular transfer); auxiliary citations (Llanos-Lagos 2025, Beattie 2014) at `governance/bibliography.md#auxiliary-citations`.
