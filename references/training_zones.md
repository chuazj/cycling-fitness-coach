...
**Non-WHOOP degradation.** When the WHOOP Recovery score is absent, `--readiness-check` reports a `signal_mode` and produces a coarser verdict: `reduced` (HRV + RHR + sleep → GREEN/YELLOW-LOW/RED, all session types still reachable) or `minimal` (sleep + subjective only). The 4-band scale (with YELLOW-HIGH) is a `full`-mode/WHOOP feature. See `workflows/advise.md` → signal-mode contract.
