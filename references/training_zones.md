...
**Non-WHOOP degradation.** When the WHOOP Recovery score is absent, `--readiness-check` reports a `signal_mode` and produces a coarser verdict: `reduced` (HRV and/or RHR + sleep → GREEN/YELLOW-LOW/RED, all session types still reachable) or `minimal` (sleep + subjective only). The 4-band scale (with YELLOW-HIGH) is a `full`-mode/WHOOP feature. See `references/cli_reference.md` → Readiness Check → Signal-mode contract.
