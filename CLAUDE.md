...
- **wellness partial_baseline guard**: When only one daily wellness record is available (no historical baseline), `wellness_summary()` sets `partial_baseline: true`, populates `baseline_note`, and suppresses RHR/HRV deviation flags. Latest-day-only flags still fire (sleep <6h, subjective ≥4, Recovery <67 — the last uses absolute Whoop bands, not deviation). Coaching output should surface `baseline_note` rather than treating the single record as a baseline.
- **Two parallel readiness templates**: Both `workflows/advise.md` (Mid-Week Check-In) and `workflows/plan.md` (Weekly Review) contain a `### Readiness (from --wellness)` output template with the same field structure. When updating wellness output fields (e.g., adding a new flag, surfacing a new metric), edit BOTH or coaching output will diverge between the two workflows.
