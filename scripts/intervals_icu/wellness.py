...
# Baseline maturity threshold — drives both deviation-flag suppression and
# the human-readable baseline_note. ≥7 days reflects the noise floor for
# HRV/RHR daily measurements (single-night swings of ±10-20% are common;
# require a week of history before treating deltas as signal). ≥14 days =
# "stable" matches HRV4Training / Marco Altini convention.
MIN_BASELINE_SIZE = 7

# Whoop-exclusive metrics — a record is counted as "days_with_whoop_data"
# when at least one of these is populated. Excludes `sleep_hours` because
# intervals.icu's UI lets athletes manually enter sleep (a record with only
# sleepSecs populated is a manual entry, not a Whoop sync). `sleep_score` IS
...
def _build_daily(wellness):
    """Normalize the raw sorted wellness records into the `daily` list of dicts."""
    daily = []
    for w in wellness:
...
def _history_value_lists(daily):
    """Compute history, per-metric value lists, sample sizes, and partial_baseline.

    Returns (history, value_lists, baseline_sample_sizes, partial_baseline).
    """
    ...
    return history, value_lists, baseline_sample_sizes, partial_baseline


def _baseline_maturity(baseline_sample_sizes):
    """Baseline-maturity tier logic. Returns (maturity_str, nonzero_sizes)."""
    ...
    return baseline_maturity, nonzero_sizes


def _hrv_baseline(hrvs):
    """HRV baseline sub-computation — returns a dict fragment of HRV baseline keys."""
    frag = {}
    if hrvs:
        frag["hrv_avg"] = round(sum(hrvs) / len(hrvs), 1)
        ...
        if hrv_sd_full is not None and frag["hrv_avg"] > 0:
            frag["hrv_cv_pct"] = round(hrv_sd_full / frag["hrv_avg"] * 100, 1)
        ...
        if recent7:
            frag["hrv_7d_mean"] = round(sum(recent7) / len(recent7), 1)
            sd7 = _stdev(recent7)
            if sd7 is not None:
                frag["hrv_7d_sd"] = round(sd7, 1)
...
def _compute_baseline(value_lists):
    """Build the full baseline dict — per-metric averages plus HRV fragment."""
    rhrs = value_lists["rhrs"]
    hrvs = value_lists["hrvs"]
    sleeps = value_lists["sleeps"]
    readinesses = value_lists["readinesses"]
    respirations = value_lists["respirations"]
    spo2s = value_lists["spo2s"]

    baseline = {}
    if rhrs:
        baseline["resting_hr_avg"] = round(sum(rhrs) / len(rhrs), 1)
    baseline.update(_hrv_baseline(hrvs))
    if sleeps:
...
def _flag_rhr(latest, baseline, sizes):
    """RHR deviation flags. Returns a list of 0-1 flag dicts."""
    flags = []
    ...
    if (latest.get("resting_hr") and baseline.get("resting_hr_avg")
            and sizes["resting_hr"] >= MIN_BASELINE_SIZE):
...
def _flag_hrv_band(latest, daily, baseline, sizes):
    """HRV 7-day-band flags (yellow/red escalation). Returns a list of 0-1 flag dicts."""
    flags = []
    ...
    if (latest.get("hrv") and mu7 is not None and sd7 is not None
            and sizes["hrv"] >= MIN_BASELINE_SIZE):
...
def _flag_hrv_cv_trend(baseline):
    """HRV CV-trend flag. Returns a list of 0-1 flag dicts."""
    flags = []
...
def _flag_respiration(latest, baseline, sizes):
    """Respiration two-tier flags. Returns a list of 0-1 flag dicts."""
    flags = []
    ...
    if (latest.get("respiration") is not None
            ...
            and sizes["respiration"] >= MIN_BASELINE_SIZE):
...
def _flag_spo2(latest, baseline, sizes):
    """SpO2 flags (baseline-relative + <90% red floor). Returns a list of 0-1 flag dicts."""
    flags = []
    ...
    if spo2_today is not None:
        ...
        spo2_mature = (spo2_base is not None
                       and sizes["spo2"] >= MIN_BASELINE_SIZE)
...
def _flag_absolute(latest):
    """Absolute-threshold flags: sleep <6h, recovery band, subjective ≥4.
...
def _recovery_slope_3day(daily):
    """3-day recovery slope. Returns (slope_dict_or_None, flag_or_None)."""
    ...
    return recovery_slope_3day, slope_flag


def _latest_date_age_days(latest):
    """Age in days of the latest record. Returns int or None."""
...
def _subjective_stale(latest):
    """Subjective-stale heuristic. Returns bool."""
    ...
    return len(subj_filled) >= 3 and all(v == 1 for v in subj_filled)


def _training_load(wellness):
    """CTL/ATL/TSB extraction from the latest raw wellness record. Returns the dict."""
...
def _progression_signal(daily, wellness, baseline, sizes):
    """Progression-signal block. Returns the progression dict or None."""
...
def _days_with_whoop_data(daily):
    """Days-with-Whoop-data count. Returns int."""
...
def _overall_status(flags):
    """Worst-of-severity rollup over flags → "red"/"yellow"/"green"."""
    ...
    return "red" if any(f["severity"] == "red" for f in flags) \
        else "yellow" if any(f["severity"] == "yellow" for f in flags) \
        else "green"


def _baseline_note(maturity, nonzero_sizes):
    """Tiered baseline-note string. Returns the note string or None."""
...
def wellness_summary(client, days=14):
    ...
    maturity, nonzero_sizes = _baseline_maturity(sizes)
    ...
    slope, slope_flag = _recovery_slope_3day(daily)
    if slope_flag:
        flags.append(slope_flag)

    return {
        "days": days,
        "days_with_data": len(daily),
        "days_with_whoop_data": _days_with_whoop_data(daily),
        ...
        "baseline": baseline,
        "latest": latest,
        "latest_date_age_days": _latest_date_age_days(latest),
        "flags": flags,
        "overall_status": _overall_status(flags),
        "daily": daily,
        "baseline_note": _baseline_note(maturity, nonzero_sizes),
        "recovery_slope_3day": slope,
        "subjective_stale_warning": _subjective_stale(latest),
        "training_load": _training_load(wellness),
        "progression_signal": _progression_signal(daily, wellness, baseline, sizes),
    }
